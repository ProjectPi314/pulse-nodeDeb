...
function common_task_software_check() {
    ...
    if [[ -z "${python_check}" || -z "${docker_check}" || -z "${docker_compose_check}" || -z "${openssl_check}" ]]; then
        ...
        # Attempt to install Python 3.10 directly from Debian repositories
        sudo apt-get install -y python3.10 python3.10-venv python3.10-dev python3-pip || {

            # Fallback: Install Python 3.10 from source if not available
            echo "Python 3.10 not available in Debian repositories. Installing from source..."
