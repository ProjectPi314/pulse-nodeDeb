...
# Get the highest installed Python3 version
HIGHEST_PY3_VERSION=$(python3 --version 2>/dev/null | awk '{print $2}' | cut -d. -f1,2)

# Check if the highest Python version meets the requirement
if [[ $(echo "$HIGHEST_PY3_VERSION < $REQUIRED_PY_VERSION" | bc -l) -eq 1 ]]; then
    ...
    # Attempt to install the required Python version directly from Debian repositories
    sudo apt-get install -y python3.8 python3.8-venv python3.8-dev || {

        # Fallback: Install Python 3.8 from source if not available
        echo "Python 3.8 not found in Debian repositories. Installing from source..."
